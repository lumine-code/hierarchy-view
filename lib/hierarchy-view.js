...
const { HIERARCHIES, OTHER_DIRECTION } = require("./hierarchies");
...
// The dock item. Renders one prepared hierarchy item as the root of a lazily
// expanded tree, in one of the two directions of one of the two hierarchies —
// see `hierarchies.js`. Children are fetched on first expansion and cached per
// node; switching direction re-queries. One item serves both hierarchies, so
// running a type command over a displayed call tree replaces it and retitles
// the tab.
class HierarchyView {
  constructor({ kind = "call", direction = "up" } = {}) {
  ...
  // The tab renders the title and the icon once and then follows these, so a
  // switch from one hierarchy to the other has to announce itself or the tab
  // goes on saying "Call Hierarchy" over a tree of supertypes.
  ...
  getTitle() {
    return HIERARCHIES[this.kind].title;
  }
  ...
  getIconName() {
    return HIERARCHIES[this.kind].iconName;
  }
  ...
  // Show `item` (a prepared hierarchy item) as the tree root, querying `kind`
  // in `direction`. `session` is the server that prepared the item, and serves
  // every request in the tree: an item's `data` is opaque and means nothing to
  // any other server.
  async showItem(session, item, kind, direction) {
    this.session = session;
    this.setHierarchy(kind, direction);
    this.nodes.clear();
    this.nodeId = 1;
  ...
  setHierarchy(kind, direction) {
    const changed = this.kind !== kind;
    this.kind = kind;
    this.direction = direction;
    if (changed) {
      this.emitter.emit("did-change-title", this.getTitle());
      this.emitter.emit("did-change-icon", this.getIconName());
    }
  }

  switchDirection() {
    this.setDirection(OTHER_DIRECTION[this.direction]);
  }

  // Re-query the current root in the other direction. Cached children are
  // direction-specific, so the tree is rebuilt from the root item. The item
  // itself is direction-independent, so this needs no second prepare.
  ...
  // Fetch the next level for `node`. Returns child nodes, or null when the
  // request failed (so a retry stays possible).
  async fetchChildren(node) {
    if (!this.session) return null;
    const { title } = HIERARCHIES[this.kind];
    const { method, key } = HIERARCHIES[this.kind][this.direction];
    let results;
    try {
      results = await this.session.request(method, { item: node.item });
    } catch (error) {
    ...
    // Unwrap by direction rather than coalescing `from ?? to`: an
    // outgoingCalls reply that wrongly carried `from` used to half-work.
    return (results ?? []).map((result) =>
      this.createNode(key ? result[key] : result, result.fromRanges, node),
    );
  }
  ...
  renderRow(node) {
    ...
    name.className = "hierarchy-view-name";
    name.textContent = node.item.name;
    this.iconDisposables.add(
      lumine.icons.applyTo(
        name,
        { kind: SYMBOL_KINDS[node.item.kind] ?? "unknown", context: "hierarchy-view" },
        { setData: false },
      ),
    );
  ...
  render() {
    this.iconDisposables.dispose();
    this.iconDisposables = new CompositeDisposable();
    const hierarchy = HIERARCHIES[this.kind];
    const direction = hierarchy[this.direction];
    this.directionLabel.className = `hierarchy-view-direction icon ${direction.icon}`;
    this.directionLabel.textContent = direction.label;
    this.switchButton.title = direction.switchTitle;
    ...
    if (!this.root) {
      ...
      line.textContent = hierarchy.empty;
