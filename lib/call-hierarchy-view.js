...
const CALL_HIERARCHY_URI = "lumine://call-hierarchy";
...
class CallHierarchyView {
  constructor(getService) {
    ...
    this.element.className = "call-hierarchy tool-panel";
    this.element.tabIndex = -1;

    this.header = document.createElement("div");
    this.header.className = "call-hierarchy-header";
    this.directionLabel = document.createElement("span");
    this.directionLabel.className = "call-hierarchy-direction icon icon-arrow-left";
    this.switchButton = document.createElement("button");
    this.switchButton.className = "call-hierarchy-switch btn btn-xs icon icon-git-compare";
    ...
    this.list.className = "call-hierarchy-list";
  ...
  getDefaultLocation() {
    return lumine.config.get("call-hierarchy.dockSide") === "left" ? "left" : "right";
  }
  ...
  // `direction` ("incoming" or "outgoing"). `editor` is the origin editor all
  // subsequent requests route through.
  async showItem(editor, item, direction) {
  ...
  async fetchChildren(node) {
    ...
    const editor = this.originEditor;
    if (!service || !editor || editor.isDestroyed()) return null;
    const method =
      this.direction === "incoming" ? "callHierarchy/incomingCalls" : "callHierarchy/outgoingCalls";
    let calls;
    try {
      calls = await service.request(editor, method, { item: node.item });
    } catch (error) {
  ...
  handleClick(event) {
    const li = event.target?.closest("li.call-hierarchy-entry");
    ...
    const name = li.firstElementChild?.querySelector(".call-hierarchy-name");
  ...
  moveDelta(delta) {
    const entries = Array.from(this.list.querySelectorAll("li.call-hierarchy-entry"));
    if (entries.length === 0) return;
    const selected = this.list.querySelector("li.call-hierarchy-entry.selected");
  ...
  renderRow(node) {
    ...
    row.className = node.leaf ? "call-hierarchy-item" : "call-hierarchy-item list-item";
    row.title = this.rowTitle(node.item);

    const name = document.createElement("span");
    name.className = `call-hierarchy-name icon ${iconForKind(node.item.kind)}`;
    name.textContent = node.item.name;
    row.appendChild(name);

    if (node.fromRanges?.length > 1) {
      const badge = document.createElement("span");
      badge.className = "call-hierarchy-count badge badge-flexible";
    ...
    if (detailText) {
      ...
      detail.className = "call-hierarchy-detail";
  ...
  renderNode(node) {
    ...
    if (node.leaf) {
      li.className = "call-hierarchy-entry list-item";
      li.appendChild(this.renderRow(node));
    } else {
      li.className = `call-hierarchy-entry list-nested-item${node.expanded ? "" : " collapsed"}`;
      li.appendChild(this.renderRow(node));
      if (node.expanded && node.children?.length) {
        const ul = document.createElement("ul");
        ul.className = "call-hierarchy-tree list-tree";
  ...
  render() {
    ...
    this.directionLabel.className = `call-hierarchy-direction icon ${incoming ? "icon-arrow-left" : "icon-arrow-right"}`;
    ...
    tree.className =
      "call-hierarchy-tree call-hierarchy-tree-root list-tree has-collapsable-children";
...
module.exports = CallHierarchyView;
